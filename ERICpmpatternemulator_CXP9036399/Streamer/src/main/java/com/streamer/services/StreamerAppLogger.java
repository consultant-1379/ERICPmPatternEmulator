...
/**
 * The Class StreamerAppLogger.
 */
@Service
public class StreamerAppLogger {

    /** The logger. */
    private static final Logger LOGGER = Logger.getLogger("com.streamer");
    ...
    public static void intializeLogger(String appPath){
        try{
            createLogBackup(appPath);
            LOGGER.setUseParentHandlers(false);
            LOGGER.setLevel(Level.FINE);

            // Creating fileHandler
            final FileHandler fileHandler = new FileHandler(appPath + File.separator + "logs/streamingLogs/ST" + ".log");
     ...
     */
    public static boolean createLogBackup(String appPath){
        try{
            String logLocation = appPath + File.separator + "logs/streamingLogs";
            ...
            Date today = new Date();
            String backupDirName = "ST_" + convertDateToDateTimeString(today);
            ...
            for(File file : files){
                if(file.isDirectory() && !file.getName().startsWith("inst")){
                    file.delete();
                }
            }
            out.close();
        } catch(Exception ex){
            System.out.println("WARN : Unable to create logs backup directory." + ex);
            return false;
        }
        return true;
    }
    ...
    private static void addDir(File dirObj, ZipOutputStream out, String backupDirName) throws IOException{
        ...
        for(int i = 0; i < files.length; i++){
            ...
            if(backupFlag){
                ...
                out.putNextEntry(new ZipEntry(backupDirName + File.separator + files[i].getPath().substring(files[i].getPath().indexOf(
                        "streamingLogs") + 5)));
            } else{
